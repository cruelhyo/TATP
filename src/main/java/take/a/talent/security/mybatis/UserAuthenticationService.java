...
public class UserAuthenticationService implements UserDetailsService 
{
	...
	/*
	 * loadUserByUsername 매서드를 통하여 DB에 있는 값을 받아 VO객체를 생성후 리턴한다
	 * 입력값으로는 폼에서 입력한 id를 입력값으로 호출한다
	 */
	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException 
	{
		...
		//ROLE_WITHDRAWAL_MEM, ROLE_RESTRICTION 권한이 아닌 나머지 권한을 가진 유저는 전부 true로 설정, 맵핑한 값을 넣은 후 리턴한다
		return new UserDetailsVO(user.get("member_id").toString()
								, user.get("member_pw").toString()
								, true
								, true, true, true
